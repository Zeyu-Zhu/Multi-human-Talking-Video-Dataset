...
from .audio_module import MultiHumanAudioBlock
...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        if use_audio_module is True: self.audio_module = nn.ModuleList([])
        ...
        for i, down_block_type in enumerate(down_block_types):
            ...
            if use_audio_module is True:
                feature_shape = (vae_latent_shape[0]//(i+1), vae_latent_shape[0]//(i+1))
                audio_module = MultiHumanAudioBlock(depth=i, in_channels=output_channel, 
                                                    out_channels=output_channel, feature_shape=feature_shape,)
                self.audio_module.append(audio_module)
        # mid
    ...
    def forward(
        ...
        if pose_cond_fea is not None: sample = sample + pose_cond_fea
        ...
        for idx, downsample_block in enumerate(self.down_blocks):
            if (hasattr(downsample_block, "has_cross_attention") and downsample_block.has_cross_attention):
                ...
                sample, res_samples = downsample_block(
                    ...
                    temb=emb,
                    encoder_hidden_states=encoder_hidden_states,)
            if self.use_audio_module is True:
                sample = self.audio_module[idx](hidden_states=sample, face_mask_list=face_mask_list,
                                                lip_mask_list=lip_mask_list, activate_score_list=activate_score_list,
                                                audio_embedding=audio_embedding, motion_scale=motion_scale,)
