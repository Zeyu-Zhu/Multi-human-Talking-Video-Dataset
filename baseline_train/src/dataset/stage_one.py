    ...
    def __getitem__(self, index):        
        while True:
            try:
                ...
                img_path = video_meta["image_dir"]
                kps_path = video_meta["pose_dir"]
                speaker_info_path = video_meta["speak_info_path"]
                ...
                start_frame, end_frame = infos[0]['clip_range']
                
                img_list = sorted([path.join(img_path, name) for name in os.listdir(img_path)])[start_frame-1:end_frame]
                pose_list = sorted([path.join(kps_path, name) for name in os.listdir(kps_path)])[start_frame-1:end_frame]
...
if __name__ == '__main__':
    datasets = HumanDanceDataset(img_size=(1080, 1920), data_meta_paths=["/users/zeyuzhu/ControlSD/Moore-AnimateAnyone/data_config_new/fallowshow_train_data_config.json"])
    sample = datasets[0]
